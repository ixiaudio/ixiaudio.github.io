...
NTP.server; // start a server, you need to do this as root in a Terminal window, so you'll be prompted for a password.

NTP.client(serverNetAddr); // start a client synced to the specified server. Ditto re password etc. This will sync immediately and then slave thereafter.
